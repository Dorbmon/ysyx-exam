...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, printf("%ld\n", src1), R(dest) = s->pc + 4, s->pc += src1);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4, s->pc = src1 + src2);
  
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, printf("a0:%ld\n", R(10)),NEMUTRAP(s->pc, R(10))); // R(10) is $a0
